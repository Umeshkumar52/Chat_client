...
export default function NabigationBar({searching,setSearching,sliderOpen,setSliderOpen}){
  ...
  return (
    <div className='relative flex min-h-[3.5rem] bg-white px-4 items-center justify-between border-b-2'>
      ...
      <div className='flex items-center gap-8 text-2xl md:pr-10'>
        {/* Search bar handle */}
        <div onClick={()=>setSearching(!searching)} className={`relative lg:w-[30vw] w-[25vw] hidden md:${searching?"hidden":"block"}`} >
         <input type="text" placeholder='Search...' className='w-full rounded-lg px-2 pr-14 py-1 focus:outline-none border-2 border-slate-800' />
         <GrSearch className='absolute right-5 bottom-2' id='search'/>
       </div>
        <GrSearch onClick={()=>setSearching(!searching)} className='md:hidden block'/>
        <CgAddR className='lg:hidden block' onClick={()=>setSliderOpen(!sliderOpen)}/>
