  ...
  return (
    ...
    <div className='w-full'>
     ...
     <div className='relative'>
      ...
      {!profileBannerUrl?
       <img className='w-full h-full' src={user.profileBanner}/>:
       <img className='w-full h-full' src={profileBannerUrl}/>
      }
    ...
    <div className='absolute flex justify-center items-center top-[5rem] left-[1rem] rounded-full'>
     ...
     {profileUrl?
      <img className='w-[8rem] h-[8rem] ring-2 rounded-full' src={profileUrl} alt="" />:
      <img className='w-[8rem] h-[8rem] ring-2 rounded-full' src={user.avatar} alt="" />
     }
     </label>
      <input onChange={profileHandler} type='file' id='profile' className='hidden' name='profile'/>
    </div>
      <div className='relative pt-[4rem] gap-2 flex flex-col px-3 pb-[1rem] border-b-2'>
     ...
     {/* post Area */}
      ...
      <hr/>
     {/* post Card */}
     <div className='hidden peer-checked/post:block'>
    {(user.myPosts.length>0)?
      user.myPosts.map((Element,index)=>{
        console.log(user.myPosts.length);
       return <MediaCard key={index} data={Element} />
      }):<div className='w-full h-full flex justify-center items-center'>No posts</div>
    }
     </div>
     {/* ReelsCard */}
     <div className='hidden peer-checked/reels:block'>
    {(user.myReels.length>0)?
      user.myReels.map((Element,index)=>{
       return <ReelsCard key={index} data={Element} />
