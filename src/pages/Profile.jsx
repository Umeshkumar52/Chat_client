...
export default function Profile() {
  const {NewUserProfile,user}=useSelector(state=>{return state.auth}) 
 ...
 async function profileBannerHandler(event){
  ...
  return (
    ...
    <div className='w-full'>
     ...
     <div className='relative'>
      ...
      {/* {profileBannerUrl || user.profileBanner? */}
       <img className='w-full h-full' src={profileBannerUrl?profileBannerUrl:userData.profileBanner}/>:
      {/* } */}
     ...
     {profileUrl || userData.avatar?
      ...
      <div className='relative pt-[4rem] gap-3 flex flex-col px-3 pb-[1rem] border-b-2'>
       <div >
        ...
        //  onClick={()=>navigate("/friendRequest",{state:userData._id})}
         }
        </div>
      </div>
     </div>
    ...
    {(userData.myPosts)?
      userData.myPosts.map((Element,index)=>{
    ...
    {(userData.myReels)?
      userData.myReels.map((Element,index)=>{
